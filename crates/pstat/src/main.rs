...
use pstat_core::collector::{Collector, DiscoverQuery, PstatError, ProcessTarget};
...
#[derive(Debug, Parser)]
#[command(name = "pstat")]
#[command(about = "Process stat collection tool — snapshot, sample, diff, discover, report")]
#[command(version)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
...
fn resolve_target(pid: Option<u32>, name: Option<String>, exe: Option<String>) -> Result<ProcessTarget> {
    if let Some(p) = pid {
...
fn parse_duration(s: &str) -> Result<Duration> {
    let parse_f64_safe = |v: &str| -> Result<f64> {
        let f: f64 = v.parse()?;
        anyhow::ensure!(f.is_finite() && f >= 0.0, "duration must be a non-negative number, got '{v}'");
        Ok(f)
    };
...
fn run() -> Result<(), PstatError> {
    ...
    match cli.command {
        Commands::Snapshot { pid, name, exe, target, format, output, verbose } => {
        ...
        Commands::Sample { pid, name, exe, target, interval, count, format, output } => {
            let proc_target = resolve_target(pid, name, exe).map_err(|e| PstatError::Other(e))?;
            ...
            let source = match &target {
                Some(addr) => pstat_core::schema::CollectionSource::Remote { target: addr.clone() },
                ...
                if let Some((prev_user, prev_sys, prev_time)) = prev_cpu {
                    ...
                    if wall_ms > 0.0 {
                        let cpu_delta = (snap.cpu_user_ms + snap.cpu_system_ms)
                            .saturating_sub(prev_user + prev_sys) as f64;
            ...
            series.summary = Some(report::compute_summary(&series));
