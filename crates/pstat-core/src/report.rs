...
fn format_snapshot_markdown(snapshot: &ProcessSnapshot, verbose: bool) -> String {
    ...
    let time = snapshot.timestamp.format("%Y-%m-%d %H:%M:%S UTC").to_string();
    ...
    if verbose {
        ...
        if let Some(sc) = snapshot.shared_clean { md.push_str(&format!("| Shared Clean | {} |\n", fmt_bytes(sc as f64))); }
        if let Some(sd) = snapshot.shared_dirty { md.push_str(&format!("| Shared Dirty | {} |\n", fmt_bytes(sd as f64))); }
        if let Some(pc) = snapshot.private_clean { md.push_str(&format!("| Private Clean | {} |\n", fmt_bytes(pc as f64))); }
        if let Some(pd) = snapshot.private_dirty { md.push_str(&format!("| Private Dirty | {} |\n", fmt_bytes(pd as f64))); }
        if let Some(r) = snapshot.referenced { md.push_str(&format!("| Referenced | {} |\n", fmt_bytes(r as f64))); }
        if let Some(a) = snapshot.anonymous { md.push_str(&format!("| Anonymous | {} |\n", fmt_bytes(a as f64))); }
        if let Some(sp) = snapshot.swap_pss { md.push_str(&format!("| Swap PSS | {} |\n", fmt_bytes(sp as f64))); }
        md.push_str(&format!("| Mem % | {:.2}% |\n", snapshot.mem_percent));
    }
    md.push_str(&format!("| CPU (user) | {} |\n", fmt_ms(snapshot.cpu_user_ms)));
    md.push_str(&format!("| CPU (system) | {} |\n", fmt_ms(snapshot.cpu_system_ms)));
    ...
    if verbose {
        if let Some(sr) = snapshot.io_syscr { md.push_str(&format!("| IO Syscalls (r) | {} |\n", sr)); }
        if let Some(sw) = snapshot.io_syscw { md.push_str(&format!("| IO Syscalls (w) | {} |\n", sw)); }
    }
    ...
    if verbose {
        md.push_str(&format!("| CtxSw (vol) | {} |\n", snapshot.ctx_switches_voluntary));
        md.push_str(&format!("| CtxSw (invol) | {} |\n", snapshot.ctx_switches_involuntary));
        if let Some(score) = snapshot.oom_score { md.push_str(&format!("| OOM Score | {} |\n", score)); }
        if let Some(adj) = snapshot.oom_score_adj {
            let label = if adj < 0 { format!("{adj} (protected)") } else { format!("{adj}") };
            md.push_str(&format!("| OOM Adj | {} |\n", label));
        }
        if let Some(ref cg) = snapshot.cgroup { md.push_str(&format!("| Cgroup | `{}` |\n", cg)); }
    }
    md
}
...
fn format_snapshot_table(snapshot: &ProcessSnapshot, verbose: bool) -> String {
    ...
    table.set_header(vec![
        Cell::new(format!("{} (PID {})", snapshot.name, snapshot.pid)).add_attribute(Attribute::Bold),
        Cell::new(format!("{state} \u{00b7} {time}")),
    ]);

    table.add_row(vec![Cell::new("Source"), Cell::new(&source)]);
    if verbose {
        table.add_row(vec![Cell::new("Cmdline"), Cell::new(snapshot.cmdline.join(" "))]);
    }

    table.add_row(vec![Cell::new("RSS"), Cell::new(fmt_bytes(snapshot.rss as f64))]);
    if let Some(pss) = snapshot.pss {
    ...
    if snapshot.vm_hwm > 0 {
        table.add_row(vec![Cell::new("Peak RSS"), Cell::new(fmt_bytes(snapshot.vm_hwm as f64))]);
    }

    if verbose {
        table.add_row(vec![Cell::new("VMS"), Cell::new(fmt_bytes(snapshot.vms as f64))]);
        table.add_row(vec![Cell::new("VMS Peak"), Cell::new(fmt_bytes(snapshot.vm_peak as f64))]);
    }

    if snapshot.vm_swap > 0 {
        table.add_row(vec![Cell::new("Swap"), Cell::new(fmt_bytes(snapshot.vm_swap as f64))]);
    }

    if verbose {
        table.add_row(vec![Cell::new("Shared"), Cell::new(fmt_bytes(snapshot.shared as f64))]);
        if let Some(sc) = snapshot.shared_clean {
            table.add_row(vec![Cell::new("Shared Clean"), Cell::new(fmt_bytes(sc as f64))]);
        }
        if let Some(sd) = snapshot.shared_dirty {
            table.add_row(vec![Cell::new("Shared Dirty"), Cell::new(fmt_bytes(sd as f64))]);
        }
        if let Some(pc) = snapshot.private_clean {
            table.add_row(vec![Cell::new("Private Clean"), Cell::new(fmt_bytes(pc as f64))]);
        }
        if let Some(pd) = snapshot.private_dirty {
            table.add_row(vec![Cell::new("Private Dirty"), Cell::new(fmt_bytes(pd as f64))]);
        }
        if let Some(r) = snapshot.referenced {
            table.add_row(vec![Cell::new("Referenced"), Cell::new(fmt_bytes(r as f64))]);
        }
        ...
        table.add_row(vec![Cell::new("Mem %"), Cell::new(format!("{:.2}%", snapshot.mem_percent))]);
    }

    table.add_row(vec![Cell::new("CPU (user)"), Cell::new(fmt_ms(snapshot.cpu_user_ms))]);
    table.add_row(vec![Cell::new("CPU (system)"), Cell::new(fmt_ms(snapshot.cpu_system_ms))]);
    if let Some(pct) = snapshot.cpu_percent {
    ...
    if verbose {
        if let Some(sr) = snapshot.io_syscr {
            table.add_row(vec![Cell::new("IO Syscalls (r)"), Cell::new(format!("{sr}"))]);
        }
        if let Some(sw) = snapshot.io_syscw {
            table.add_row(vec![Cell::new("IO Syscalls (w)"), Cell::new(format!("{sw}"))]);
        }
    }

    table.add_row(vec![Cell::new("Threads"), Cell::new(format!("{}", snapshot.num_threads))]);
    if let Some(fds) = snapshot.num_fds {
        table.add_row(vec![Cell::new("FDs"), Cell::new(format!("{fds}"))]);
    }

    if verbose {
        table.add_row(vec![Cell::new("CtxSw (vol)"), Cell::new(format!("{}", snapshot.ctx_switches_voluntary))]);
        table.add_row(vec![Cell::new("CtxSw (invol)"), Cell::new(format!("{}", snapshot.ctx_switches_involuntary))]);
...
pub fn format_diff_table(report: &DiffReport) -> String {
    ...
    for d in &report.deltas {
        ...
        let severity_marker = match d.severity {
            ...
            DiffSeverity::Minor => {
                if d.delta.abs() < f64::EPSILON { "" } else if d.delta > 0.0 { "\u{25b2}" } else { "\u{25bc}" }
            }
        };
        ...
        let (b, a, delta) = if is_mem {
            (fmt_bytes(d.before), fmt_bytes(d.after), fmt_delta_val(d.delta, d.delta >= 0.0))
        } else if is_cpu_time {
            (fmt_ms(d.before as u64), fmt_ms(d.after as u64), fmt_delta_val(d.delta, d.delta >= 0.0))
        } else {
            (
                format!("{:.0}", d.before),
                format!("{:.0}", d.after),
...
pub fn compute_summary(series: &SampleSeries) -> SampleSummary {
    ...
    if n == 0 {
        return SampleSummary {
            ...
            vm_hwm_max: 0.0,
            vm_swap_max: 0.0,
            cpu_percent: StatBucket::from_values(&[]),
            io_read_rate: StatBucket::from_values(&[]),
            io_write_rate: StatBucket::from_values(&[]),
            num_threads: StatBucket::from_values(&[]),
            num_fds: StatBucket::from_values(&[]),
    ...
    let fd_vals: Vec<f64> = samples.iter().filter_map(|s| s.num_fds.map(|f| f as f64)).collect();

    // Compute IO rates between consecutive samples.
    let mut io_read_rates = Vec::new();
    let mut io_write_rates = Vec::new();
    for i in 1..n {
        let dt = (samples[i].timestamp - samples[i - 1].timestamp).num_milliseconds() as f64 / 1000.0;
        if dt > 0.0 {
            if let (Some(rb1), Some(rb2)) = (samples[i - 1].io_read_bytes, samples[i].io_read_bytes) {
                io_read_rates.push((rb2 as f64 - rb1 as f64) / dt);
            }
            if let (Some(wb1), Some(wb2)) = (samples[i - 1].io_write_bytes, samples[i].io_write_bytes) {
                io_write_rates.push((wb2 as f64 - wb1 as f64) / dt);
            }
        }
    }

    let vm_hwm_max = samples.iter().map(|s| s.vm_hwm as f64).fold(0.0f64, f64::max);
    let vm_swap_max = samples.iter().map(|s| s.vm_swap as f64).fold(0.0f64, f64::max);
    ...
    SampleSummary {
        ...
        vm_hwm_max,
        vm_swap_max,
        cpu_percent: StatBucket::from_values(&cpu_vals),
        io_read_rate: StatBucket::from_values(&io_read_rates),
        io_write_rate: StatBucket::from_values(&io_write_rates),
        num_threads: StatBucket::from_values(&thread_vals),
        num_fds: StatBucket::from_values(&fd_vals),
...
pub fn format_markdown(series: &SampleSeries) -> String {
    let summary = series.summary.as_ref().map_or_else(|| compute_summary(series), |s| s.clone());
    ...
    md.push_str(&format!(
        "| CPU % | {:.1}% | {:.1}% | {:.1}% | {:.1}% |\n",
        summary.cpu_percent.min, summary.cpu_percent.max, summary.cpu_percent.avg, summary.cpu_percent.p95
    ));

    md.push_str("\n## IO\n\n");
    md.push_str("| Metric | Avg Rate |\n");
    md.push_str("|--------|----------|\n");
    md.push_str(&format!("| Read | {}/s |\n", fmt_bytes(summary.io_read_rate.avg)));
    md.push_str(&format!("| Write | {}/s |\n", fmt_bytes(summary.io_write_rate.avg)));
    ...
    match summary.rss_trend {
        Trend::Rising => {
            md.push_str(&format!(" (+{} over {:.0}s) ... potential leak", fmt_bytes(rss_delta), summary.duration_s));
        }
        Trend::Falling => {
            md.push_str(&format!(" (-{} over {:.0}s)", fmt_bytes(rss_delta), summary.duration_s));
        }
        Trend::Stable => {}
    }
    md.push('\n');

    if summary.cpu_percent.max > 0.0 {
        let burst_ratio = if summary.cpu_percent.avg > 0.0 {
            summary.cpu_percent.p95 / summary.cpu_percent.avg
        } else {
            0.0
        };
        if burst_ratio > 2.0 {
            ...
            md.push_str(&format!(
                "- CPU: avg {:.1}%, peak {:.1}%\n",
                summary.cpu_percent.avg, summary.cpu_percent.max
            ));
        }
    }

    if summary.io_write_rate.avg > 0.0 || summary.io_read_rate.avg > 0.0 {
        let read_write = if summary.io_write_rate.avg > summary.io_read_rate.avg * 2.0 {
            "write-heavy"
        } else if summary.io_read_rate.avg > summary.io_write_rate.avg * 2.0 {
            "read-heavy"
        } else {
            "balanced"
        };
...
pub fn format_report_table(series: &SampleSeries) -> String {
    ...
    let mut out = String::new();
    out.push_str(&format!("┌─ {} (PID {}) ── report\n", series.process_name, series.pid));
    ...
    stats_table.add_row(vec![
        Cell::new("CPU %"),
        Cell::new(format!("{:.1}%", summary.cpu_percent.min)).set_alignment(CellAlignment::Right),
        Cell::new(format!("{:.1}%", summary.cpu_percent.max)).set_alignment(CellAlignment::Right),
        Cell::new(format!("{:.1}%", summary.cpu_percent.avg)).set_alignment(CellAlignment::Right),
        Cell::new(format!("{:.1}%", summary.cpu_percent.p95)).set_alignment(CellAlignment::Right),
    ]);
    stats_table.add_row(vec![
        Cell::new("IO Read"),
        Cell::new(""), Cell::new(""),
        Cell::new(format!("{}/s", fmt_bytes(summary.io_read_rate.avg))).set_alignment(CellAlignment::Right),
        Cell::new(""),
    ]);
    stats_table.add_row(vec![
        Cell::new("IO Write"),
        Cell::new(""), Cell::new(""),
        Cell::new(format!("{}/s", fmt_bytes(summary.io_write_rate.avg))).set_alignment(CellAlignment::Right),
        Cell::new(""),
    ]);
    stats_table.add_row(vec![
        Cell::new("Threads"),
    ...
    stats_table.add_row(vec![
        Cell::new("FDs"),
        Cell::new(format!("{:.0}", summary.num_fds.min)).set_alignment(CellAlignment::Right),
        Cell::new(format!("{:.0}", summary.num_fds.max)).set_alignment(CellAlignment::Right),
        Cell::new(format!("{:.1}", summary.num_fds.avg)).set_alignment(CellAlignment::Right),
        Cell::new(format!("{:.0}", summary.num_fds.p95)).set_alignment(CellAlignment::Right),
    ]);
    out.push_str(&format!("\n{stats_table}\n"));
